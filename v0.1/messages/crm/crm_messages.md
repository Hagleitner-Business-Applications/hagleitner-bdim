...
We distinguish between two categories: **events** and **commands**.
...
commands, on the other hand, are used to communicate an action or task to another explicitly addressed application. For example, the action "Create Customer in ERP" is used by CRM (Dynamics 365 CE) to instruct ERP (Dynamics 365 Business Central) to create a customer that already exists in CRM.
...
Used to receive commands for CRM. An application publishing messages to this queue instructs CRM to perform a specific action.
